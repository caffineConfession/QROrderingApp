...
import { User, Phone, Mail, CreditCard, CircleDollarSign, Send } from 'lucide-react';

const checkoutSchema = z.object({
  name: z.string().min(2, { message: "Name must be at least 2 characters." }),
  phone: z.string().regex(/^\d{10}$/, { message: "Please enter a valid 10-digit phone number." }),
  email: z.string().email({ message: "Please enter a valid email address." }),
  paymentMethod: z.enum(["Cash", "Razorpay"], { required_error: "Please select a payment method." }),
});
...
interface CheckoutFormProps {
  totalAmount: number;
  onSubmitOrder: (customerDetails: CustomerDetails, paymentMethod: PaymentMethod) => void;
  onBackToCart: () => void;
}

export default function CheckoutForm({ totalAmount, onSubmitOrder, onBackToCart }: CheckoutFormProps) {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSubmit)}>
          ...
          <CardFooter className="flex flex-col sm:flex-row justify-between gap-3 p-4">
            <Button type="button" variant="outline" onClick={onBackToCart} className="w-full sm:w-auto">
              Back to Cart
            </Button>
            <Button type="submit" className="w-full sm:w-auto" size="lg">
             <Send className="mr-2 h-5 w-5" /> Confirm Order
            </Button>
          </CardFooter>
        </form>
      </Form>
